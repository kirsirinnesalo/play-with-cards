...
public class DealingGameGUI extends Application {
    ...
    private static final int TABLE_HEIGHT = 680;
    ...
    private VBox createDeckComponent() {
        Insets padding = new Insets(50, 10, 10, 10);
        ...
        DeckPane deckPane = new DeckPane(game.getDeck(), false, CARD_WIDTH + 50, CARD_HEIGHT + 60);
        ...
        helpText.setPadding(new Insets(100, 0, 0, 0));
    ...
    private GridPane createPlayerPanes() {
        ...
        game.getPlayers().forEach(player -> {
            Pane playerPane = createPaneFor(player);
            GridPane.setMargin(playerPane, new Insets(3, 0, 1, 3));
    ...
    private Pane createPaneFor(Player player) {
        int paneWidth = CARD_WIDTH * CARDS_PER_HAND + 40;
        int paneHeight = CARD_HEIGHT * 3 + 6;
        return new PlayerPane(player, paneWidth, paneHeight, CARD_WIDTH, CARD_HEIGHT);
    }
